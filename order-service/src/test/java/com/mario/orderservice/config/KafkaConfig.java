...
public class KafkaConfig {
    ...
    @Bean
    public KafkaTemplate<String, String> kafkaTemplateString(KafkaProperties kafkaProperties) {
        return new KafkaTemplate<>(new DefaultKafkaProducerFactory<>(producerPropsString(kafkaProperties)));
    }
    ...
    public Map<String, Object> producerPropsString(KafkaProperties config) {
        var props = new HashMap<String, Object>();
        props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, config.getBootstrapServers());
        props.put(CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, config.getSecurityProtocol());
        props.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
