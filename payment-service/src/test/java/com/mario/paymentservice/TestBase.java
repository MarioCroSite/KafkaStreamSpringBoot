...
public class TestBase {
    ...
    public static List<ConsumerRecord<String, OrderFullEvent>> paymentOrdersEventTopic = new ArrayList<>();
    ...
    @KafkaListener(
            topics = {"${com.mario.kafka.payment-orders}", "error-topic"},
            groupId = "test-payment-group",
            autoStartup = "true",
            containerFactory = "listenerFactory"
    )
    public void listener(ConsumerRecord<String, ?> consumerRecord) {
        if(consumerRecord.topic().equals(kafkaProperties.getPaymentOrders())) {
            paymentOrdersEventTopic.add((ConsumerRecord<String, OrderFullEvent>) consumerRecord);
        }
